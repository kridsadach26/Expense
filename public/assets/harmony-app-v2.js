...
function cloudStateSnapshot(){
  return {schemaVersion:2,entries,appSettings,profiles,categories,budgets,recurringItems,creditCards,projects,calendarItems,appNotifications};
}
...
function initializeStaticUI(){
  if(!staticUiInitialized){
    ...
    if($('entryCurrency')){$('entryCurrency').innerHTML=currencyOptions('THB');$('entryCurrency').value='THB'}
...
function filteredEntries(){
  ...
  return entries.filter(e=>{const date=String(e.date||'');const hay=[e.merchant,e.category,e.note,e.payer].filter(Boolean).join(' ').toLowerCase();return (!q||hay.includes(q))&&(!type||e.type===type)&&(!start||date>=start)&&(!end||date<=end)}).sort((a,b)=>String(b.date||'').localeCompare(String(a.date||''))||String(b.createdAt||'').localeCompare(String(a.createdAt||'')));
}
...
$('backupBtn').onclick=()=>{
  const data={version:2,exportedAt:new Date().toISOString(),entries,appSettings,profiles,categories,budgets,recurringItems,creditCards,projects,calendarItems,appNotifications};
...
function calendarItemHtml(i,ds){const overdue=ds<today()&&i.status!=='done';return `<div class="agenda-item ${overdue?'overdue':''} ${i.status==='done'?'done':''}"><div style="display:flex;justify-content:space-between;gap:8px"><div><strong>${i.type==='bill'?'💳':i.type==='task'?'✅':'📌'} ${escapeHtml(i.name)}</strong><div class="muted">${i.time||'ทั้งวัน'} · ${(i.assignees||[]).map(a=>profiles[a]?.name||a).join(', ')||'ไม่ได้ระบุผู้รับผิดชอบ'}</div>${i.note?`<div class="small">${escapeHtml(i.note)}</div>`:''}</div>${i.amount?`<strong>${money(i.amount)}</strong>`:''}</div><div class="actions" style="margin-top:9px">${i.status!=='done'?`<button class="btn btn-primary" onclick="completeCalendarItem('${i.id}','${ds}')">${i.type==='bill'?'ชำระแล้ว':'เสร็จแล้ว'}</button>`:'<span class="badge in">เสร็จแล้ว</span>'}<button class="btn btn-light" onclick="openCalendarEditor('${i.id}')">แก้ไข</button></div></div>`}
function openCalendarEditor(id=''){
  const i=calendarItems.find(x=>x.id===id);$('calendarEditId').value=id;$('calendarEditorTitle').textContent=i?'แก้ไขกำหนดการ':'เพิ่มกำหนดการ';$('calendarName').value=i?.name||'';$('calendarType').value=i?.type||'bill';$('calendarPriority').value=i?.priority||'normal';$('calendarDate').value=i?.date||selectedCalendarDate||today();$('calendarTime').value=i?.time||'';$('calendarAmount').value=i?.amount||'';$('calendarCategory').innerHTML=categories.map(c=>`<option ${c===(i?.category||'อื่น ๆ')?'selected':''}>${c}</option>`).join('');$('calendarRepeat').value=i?.repeat||'none';$('calendarRemind').value=String(i?.remindDays??1);$('calendarPayment').value=i?.payment||'โอนเงิน';$('calendarNote').value=i?.note||'';$('calendarProject').innerHTML='<option value="">ไม่เชื่อมโปรเจกต์</option>'+projects.map(p=>`<option value="${p.id}" ${p.id===i?.projectId?'selected':''}>${escapeHtml(p.name)}</option>`).join('');$('calendarAssignees').innerHTML=Object.keys(profiles).map(k=>`<label><input type="checkbox" value="${k}" ${(i?.assignees||[activeUser]).includes(k)?'checked':''}> ${escapeHtml(profiles[k]?.name||k)}</label>`).join('');$('deleteCalendarBtn').classList.toggle('hidden',!i);calendarEditorDialog.showModal();
}
$('calendarForm').onsubmit=e=>{e.preventDefault();const id=$('calendarEditId').value||crypto.randomUUID();const old=calendarItems.find(x=>x.id===id);const item={id,name:$('calendarName').value.trim(),type:$('calendarType').value,priority:$('calendarPriority').value,date:$('calendarDate').value,time:$('calendarTime').value,amount:Number($('calendarAmount').value||0),category:$('calendarCategory').value,repeat:$('calendarRepeat').value,remindDays:Number($('calendarRemind').value||0),assignees:[...$('calendarAssignees').querySelectorAll('input:checked')].map(x=>x.value),projectId:$('calendarProject').value,payment:$('calendarPayment').value,note:$('calendarNote').value.trim(),status:old?.status||'pending',completedOccurrences:old?.completedOccurrences||[],createdBy:old?.createdBy||activeUser,createdAt:old?.createdAt||new Date().toISOString()};if(old)calendarItems=calendarItems.map(x=>x.id===id?item:x);else calendarItems.unshift(item);queueCloudSave();calendarEditorDialog.close();renderCalendar();createNotification(item.assignees,`เพิ่มกำหนดการ: ${item.name}`,item.date,item.id)};
$('deleteCalendarBtn').onclick=()=>{const id=$('calendarEditId').value;if(confirm('ลบกำหนดการนี้หรือไม่?')){calendarItems=calendarItems.filter(x=>x.id!==id);queueCloudSave();calendarEditorDialog.close();renderCalendar()}};
function completeCalendarItem(id,occurrenceDate){const i=calendarItems.find(x=>x.id===id);if(!i)return;if(i.repeat&&i.repeat!=='none'){i.completedOccurrences=[...(i.completedOccurrences||[]),occurrenceDate]}else i.status='done';if(i.type==='bill'&&i.amount){entries.unshift({id:crypto.randomUUID(),date:occurrenceDate,type:'expense',amount:i.amount,baseAmount:i.amount,currency:'THB',exchangeRate:1,category:i.category||'อื่น ๆ',payer:activeUser,payment:i.payment||'โอนเงิน',merchant:i.name,note:'สร้างจากปฏิทิน',images:[],createdAt:new Date().toISOString()})}createNotification(i.assignees,`${i.type==='bill'?'ชำระ':'ทำงาน'}แล้ว: ${i.name}`,occurrenceDate,i.id);queueCloudSave();renderAll()}
function createNotification(users,title,date,itemId){if(!(users||[]).includes(activeUser))return;const n={id:crypto.randomUUID(),title,date,itemId,read:false,createdAt:new Date().toISOString()};appNotifications.unshift(n);showToast(title);sendBrowserNotification(title,date);queueCloudSave();updateNotificationBadge()}
